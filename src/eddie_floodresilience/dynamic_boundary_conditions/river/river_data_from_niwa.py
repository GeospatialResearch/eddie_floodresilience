...
async def fetch_rec_data(
    ...
    # Using manual decompression since aiohttp can struggle with brotli compression
    async with session.get(url, params=query_param, auto_decompress=False) as resp:
        # Process the compressed response as JSON
        content_bytes_compressed = await resp.content.read()
        content_bytes = brotli.decompress(content_bytes_compressed)
