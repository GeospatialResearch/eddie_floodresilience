...
def get_valid_parameters_based_on_confidence_level() -> Dict[str, Dict[str, Union[str, int]]]:
    ...
    column_names_query = text(r"""
        ...
        WHERE
            ...
            AND column_name ~ '^p\d+'
        """).bindparams(db_name=config.EnvVariable.POSTGRES_DB)
...
def main(
    ...
    ----------
    ValueError
        Raised when the supplied sea-level rise parameters are not valid with each other. Error will state more details.
    """
    try:
