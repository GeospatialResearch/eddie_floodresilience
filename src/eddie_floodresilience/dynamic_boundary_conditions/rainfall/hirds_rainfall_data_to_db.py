...
def get_site_ids_not_in_db(conn: Connection, site_ids_in_catchment: List[str], idf: bool) -> List[str]:
    """
    ...
    site_ids_in_db = pd.read_sql_query(query, conn)
...
def add_rainfall_data_to_db(conn: Connection, site_id: str, idf: bool) -> None:
    """
    ...
    for block_structure in layout_structure:
        ...
        rain_data.to_sql(rain_table_name, conn, index=False, if_exists="append")


def add_each_site_rainfall_data(conn: Connection, site_ids_list: List[str], idf: bool) -> None:
    """
    ...
    for site_id in site_ids_list:
        add_rainfall_data_to_db(conn, site_id, idf)


def rainfall_data_to_db(conn: Connection, sites_in_catchment: gpd.GeoDataFrame, idf: bool = False) -> None:
    """
    ...
    if tables.check_table_exists(conn, table_name):
        ...
        site_ids_not_in_db = get_site_ids_not_in_db(conn, site_ids_in_catchment, idf)
        # Check if there are sites not in the database
        if site_ids_not_in_db:
            # Add rainfall data for sites not in the database
            add_each_site_rainfall_data(conn, site_ids_not_in_db, idf)
        else:
        ...
        if site_ids_in_catchment:
            ...
            add_each_site_rainfall_data(conn, site_ids_in_catchment, idf)
        else:
